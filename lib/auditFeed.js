...
function auditEvents(eventsByBook) {
  ...
  const allSlots = []; const pairStats = {};
  ...
  for (const g of groups) {
    ...
    for (const [book, { q }] of Object.entries(g.byBook)) {
      ...
      for (let i = 1; i < tl.length; i++) { const a = q.totals[tl[i - 1]], b = q.totals[tl[i]];
        if (a.over && b.over && b.over < a.over * 0.99) { P.totalsLadderViolations++; if (P.examples.length < 6) P.examples.push({ match: label, issue: 'totals out of order: Over ' + tl[i - 1] + '=' + a.over + ' but Over ' + tl[i] + '=' + b.over }); }
        if (a.under && b.under && b.under > a.under * 1.01) { P.totalsLadderViolations++; if (P.examples.length < 6) P.examples.push({ match: label, issue: 'totals out of order: Under ' + tl[i - 1] + '=' + a.under + ' but Under ' + tl[i] + '=' + b.under }); } }
      ...
      for (let i = 1; i < sl.length; i++) { const a = q.spreads[sl[i - 1]], b = q.spreads[sl[i]]; if (a.home && b.home && b.home > a.home * 1.01) { P.spreadsLadderViolations++; if (P.examples.length < 6) P.examples.push({ match: label, issue: 'handicap out of order: home ' + sl[i - 1] + '=' + a.home + ' but home ' + sl[i] + '=' + b.home }); } }
    ...
    for (const [book, { q }] of Object.entries(g.byBook)) {
      if (q.h2h) for (const [sd, p] of Object.entries(q.h2h)) slots.push({ type: 'h2h', key: 'h2h', side: sd, book, price: p });
      for (const [L, t] of Object.entries(q.totals)) for (const [sd, p] of Object.entries(t)) slots.push({ type: 'totals', key: 'totals_' + L, side: sd, book, price: p });
      for (const [L, t] of Object.entries(q.spreads)) for (const [sd, p] of Object.entries(t)) slots.push({ type: 'spreads', key: 'spreads_' + L, side: sd, book, price: p });
    }
    allSlots.push({ label, slots });
    // how often do two books quote EXACTLY the same price for the same selection?
    for (const x of slots) for (const y of slots) {
      if (x.book >= y.book || x.key !== y.key || x.side !== y.side) continue;
      const k = x.book + '|' + y.book;
      const ps = pairStats[k] = pairStats[k] || { shared: 0, identical: 0 };
      ps.shared++; if (Math.abs(x.price - y.price) < 1e-9) ps.identical++;
    }
  }

  // Books that carry the same prices are one feed under two names: report it, and never let a
  // book "corroborate" its own twin.
  const twins = {}; const identicalFeeds = [];
  for (const [k, ps] of Object.entries(pairStats)) {
    if (ps.shared >= 50 && ps.identical / ps.shared >= 0.98) {
      const [a, b] = k.split('|');
      (twins[a] = twins[a] || new Set()).add(b); (twins[b] = twins[b] || new Set()).add(a);
      identicalFeeds.push(a + ' and ' + b + ' quote identical prices on ' + ps.identical + ' of ' + ps.shared + ' shared selections — one feed under two names');
    }
  }
  for (const { label, slots } of allSlots) {
    for (const x of slots) {
      const others = slots.filter(y => y.key === x.key && y.side === x.side && y.book !== x.book && !(twins[x.book] && twins[x.book].has(y.book))).map(y => y.price);
