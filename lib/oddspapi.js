...
//
// For each dropped group we also capture the hostname each record's fixturePath
// points at (e.g. "22bet.com" vs "22bet.com.gh") and its bookmakerFixtureId, so
// it's possible to check by hand whether OddsPapi is actually conflating two
// different regional sites under one bookmaker key — rather than just guessing.
...
async function fetchOddsPapiOdds(bookmaker, tournamentId, sportId, sportKey, opts = {}) {
  ...
  try {
    ...
    const normalised = raw
      ...
      .filter(Boolean);
    const step1 = dropAmbiguousFixtures(normalised, bookmaker);
    const step2 = dropMismatchedFixtureIds(step1.kept, bookmaker);
    const step3 = dropUnstableFixtures(step2.kept, bookmaker, !usedCache);
    const events = step3.kept;
    const duplicatesDropped = step1.dropped, idMismatchDropped = step2.dropped, unstableDropped = step3.dropped;
    const duplicatesDetail = step1.droppedDetail;
