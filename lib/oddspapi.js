...
// 1X2, but also "1X2 - 1UP / 2UP" (early payout), half/period result markets,
// corner/card markets, team totals, etc. Mapping them all to h2h/totals/spreads
// made findArbs() treat e.g. a half-time or early-payout price as the full-time
// price, and because it keeps the HIGHEST price per side, those variant prices
// (which are often bigger) always won. Only the plain full-time market may
// feed h2h / totals / spreads.
const VARIANT_MARKET_RE = /half|1st|2nd|first|second|corner|card|booking|team|\btotal [12]\b|\b[12]up\b|early|payout|extra|overtime|penalt|minute|period|quarter|interval|exact|odd\/even|odd or even|margin|\bnext\b|race|\bboth\b|combo|\band\b|&|\+/i;
const CORE_TYPES = new Set(['1x2', 'totals', 'spreads']);
const _skippedVariantNames = new Set();
const _loggedCore = new Set();
function isVariantMarket(marketDef) {
  if (!CORE_TYPES.has(marketDef.marketType)) return false;
  const name = String(marketDef.marketName || '');
  if (VARIANT_MARKET_RE.test(name)) {
    if (_skippedVariantNames.size < 40) _skippedVariantNames.add(marketDef.marketId + ':' + name);
    return true;
  }
  return false;
}
...
function normaliseFixture(fixture, bookmaker, sportKey, marketsMap, participantsMap) {
  try {
    ...
    for (const [marketIdStr, marketData] of Object.entries(bookOdds.markets || {})) {
      ...
      if (isVariantMarket(marketDef)) continue; // half/1UP/corner/team-total etc — never the full-time market
