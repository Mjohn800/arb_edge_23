...
function findLegPrice(fe, book, arb, leg) {
  ...
  const found = [];
  for (const mkt of (bm && bm.markets) || []) {
    if (mkt.key !== leg.marketKey) continue;
    for (const o of mkt.outcomes || []) {
      if (!(o.price > 1)) continue;
      const pointOk = typeof o.point === 'number' && Math.abs(o.point - leg.point) < 1e-9;
      if (leg.marketKey === 'totals' && String(o.name).trim().toLowerCase() === leg.side && pointOk) found.push(o.price);
      else if (leg.marketKey === 'spreads' && sideOf(o.name, fe) === leg.side && pointOk) found.push(o.price);
      else if (leg.marketKey === 'h2h' && sideOf(o.name, fe) === leg.side) found.push(o.price);
    }
  }
  if (found.length === 0) return null;
  // Several different prices for the same selection = feed mixed markets; don't guess.
  if (found.some(p => Math.abs(p - found[0]) > PRICE_TOL)) return { ambiguous: true };
  return found[0];
}
...
async function verifyArbLegs(arb, fetchers) {
  ...
  const legs = arb.outcomes.map(o => {
    ...
    if (typeof now === 'object') return { ...base, status: 'ambiguous', now: null };
    return { ...base, status: Math.abs(now - o.odds) <= PRICE_TOL ? 'confirmed' : 'moved', now };
  });

  const unavailable = legs.some(l => ['selection_gone', 'event_not_found', 'error', 'ambiguous'].includes(l.status));
